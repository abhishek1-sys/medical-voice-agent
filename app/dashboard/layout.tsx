    ...
    <div className="min-h-screen bg-[#f7f8fc] flex" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <aside className="hidden lg:flex w-64 bg-[#1a2d5a] flex-col fixed top-0 left-0 h-full z-40">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          <div className="text-xs font-semibold uppercase tracking-widest text-slate-500 mb-3 px-2">Navigation</div>
          {[
            { href: '/dashboard', label: 'Dashboard', icon: <LayoutDashboard className="h-4.5 w-4.5" /> },
            { href: '/dashboard/sessions', label: 'Sessions', icon: <Activity className="h-4.5 w-4.5" /> },
            { href: '/dashboard/reports', label: 'Reports', icon: <FileText className="h-4.5 w-4.5" /> },
          ].map(link => (
            <Link
              key={link.href}
              href={link.href}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-300 hover:bg-white/10 hover:text-white text-sm font-medium transition-all group"
            >
